...
// Example: Rectangle vs Rectangle Collision
// This example demonstrates proper collision detection and resolution between rectangles
...
pub fn main() !void {
    ...
    var game_engine = try ze.core.Engine.init(allocator, 800, 600, "ZigEngine Rectangle Collision Example", options);
    ...
    // // Add a large test rectangle to clearly show rectangle collisions
    // _ = try game_engine.physics_world.static().rectangle(.{
    ...
    // Add a dynamic rectangle that will bounce off other objects
    _ = try game_engine.physics_world.dynamic().rectangle(.{
        ...
        .restitution = 0.7,
        .friction = 0.1,
        .initial_force = ze.math.Vector2.zero(),
        .velocity = ze.math.Vector2.init(0, 0), // Initial velocity
    });
...
fn handleInput(ctx: *anyopaque, eng: *ze.core.Engine) !void {
    ...
    // Check for keyboard input to create rectangles
    if (rl.isKeyPressed(rl.KeyboardKey.space)) {
        // Launch a new rectangle from the mouse position
        const mouse_pos = rl.getMousePosition();
        const size = 20 + @as(f32, @floatFromInt(rl.getRandomValue(0, 15)));
    ...
    if (rl.isKeyPressed(rl.KeyboardKey.h)) {
        std.debug.print("\n--- Controls ---\n", .{});
        std.debug.print("SPACE: Launch a new rectangle at mouse position\n", .{});
        std.debug.print("LEFT/RIGHT: Apply horizontal force\n", .{});
        std.debug.print("UP: Apply upward force (jump)\n", .{});
        std.debug.print("A/D: Apply counter-clockwise/clockwise torque\n", .{});
...
fn update(ctx: *anyopaque, eng: *ze.core.Engine, dt: f32) !void {
    ...
    // Draw instructions
    rl.drawText("SPACE: Launch rectangle | LEFT/RIGHT: Move | UP: Jump | A/D: Rotate | R: Reset | H: Help", 10, 10, 20, rl.Color.white);
}
